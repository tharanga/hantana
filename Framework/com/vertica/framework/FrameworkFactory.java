...
public class FrameworkFactory extends TransformFunctionFactory
{
	...
	Class<?> customDataFormatter = null;
	Class<?> customDestinationFormatter = null;
	Method transformRecord;
	Method transformColumnTypes;
	Method toDestination;
	Method setupFormatter;
	Method closeFormatter;
	Object dataFormatter;
	Object destinationFormatter;
	...
	public class Framework extends TransformFunction
	{
		...
		public void setup(ServerInterface srvInterface, SizedColumnTypes argTypes){
			...
			try{
				...
				//customDataFormatter = Class.forName("com.vertica.sdk." + userInput);
				customDataFormatter = Class.forName("com.vertica.sdk." + userInputDataFormatter);
				customDestinationFormatter = Class.forName("com.vertica.sdk." + userInputDestinationFormatter);
				
				transformColumnTypes = customDataFormatter.getDeclaredMethod("transformColumnTypes", ArrayList.class);
				transformRecord = customDataFormatter.getDeclaredMethod("transformRecord", ArrayList.class);
				dataFormatter = customDataFormatter.newInstance();
				
				toDestination = customDestinationFormatter.getDeclaredMethod("toDestination", String.class);
				setupFormatter = customDestinationFormatter.getDeclaredMethod("setupFormatter");
				closeFormatter = customDestinationFormatter.getDeclaredMethod("closeFormatter");
				destinationFormatter = customDestinationFormatter.newInstance();
				
				setupFormatter.invoke(destinationFormatter);
				...
				tempStr = (String)transformColumnTypes.invoke(dataFormatter, columnTypes);
				
				//Send to DestinationFormatter
				toDestination.invoke(destinationFormatter, tempStr);
				
				
			} catch(Exception e){
				//throw new  IllegalArgumentException("Could not find class name " + userInput);
		...
		public void destroy (ServerInterface srvInterface, SizedColumnTypes argTypes){ 
			try{
				closeFormatter.invoke(destinationFormatter);
			} catch(Exception e){
				//throw new  IllegalArgumentException("Could not find class name " + userInput);
		...
		PartitionWriter outputWriter) throws UdfException, DestroyInvocation
		{
			...
			do {
				...
				try{
					...
					//Send to DataFormatter
					tempStr = (String)transformColumnTypes.invoke(dataFormatter, record);
					
					//Send to DestinationFormatter
					toDestination.invoke(destinationFormatter, tempStr);
					
				}catch (Exception e){
					throw new IllegalArgumentException("Error in proccessPartition " + e);
				}
